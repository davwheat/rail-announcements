...
import CustomAnnouncementPane, {
  ICustomAnnouncementPreset,
} from "@components/PanelPanes/CustomAnnouncementPane";
...
const announcementPresets: Readonly<
  Record<string, ICustomAnnouncementPreset[]>
> = {
  stopped: [
    {
...
export default class ThameslinkClass700 extends TrainAnnouncementSystem {
  ...
  private readonly OtherServicesAvailable = [
    { title: "Other NR services", value: "other national rail services" },
  ];

  private async playApproachingStationAnnouncement(
    options: IApproachingStationAnnouncementOptions,
    ...
    if (options.terminatesHere) {
      ...
      files.push(
        ...
        {
          id: `stations.high.${options.stationCode}`,
          opts: { delayStart: 500 },
        },
      ...
      files.push("we will shortly be arriving at", {
        id: `stations.low.${options.stationCode}`,
        opts: { delayStart: 500 },
      });
    }

    if (
      Object.keys(this.StationsWithForcedChangeHere).includes(
        options.stationCode
      )
    ) {
      const manualChangeFor =
        this.StationsWithForcedChangeHere[
          options.stationCode as keyof typeof this.StationsWithForcedChangeHere
        ];
    ...
    if (
      Object.keys(this.StationsWithAttractions).includes(options.stationCode)
    ) {
      files.push("exit here for");
      files.push(
        `station attractions.${
          this.StationsWithAttractions[options.stationCode]
        }`
      );
    }
    ...
    if (options.takeCareAsYouLeave) {
      files.push({
        id: "please make sure you have all your belongings and take care as you leave the train",
        opts: { delayStart: 500 },
      });
    }
    ...
    if (options.isAto) {
      files.push({
        id: "the doors will open automatically at the next station",
        opts: { delayStart: 500 },
      });
    }
  ...
  private async playStoppedAtStationAnnouncement(
    ...
    files.push(
      {
        id: "this station is",
        opts: { delayStart: options.mindTheGap ? 500 : 0 },
      },
    ...
    if (thisStationCode === terminatesAtCode) {
      files.push({
        id: "this train terminates here all change",
        opts: { delayStart: 4600 },
      });
    } else if (callingAtCodes.length === 0) {
      if (!this.validateStationExists(terminatesAtCode, "high")) return;

      files.push(
        { id: "the next station is", opts: { delayStart: 4600 } },
      ...
      files.push(
        { id: "this train terminates at", opts: { delayStart: 4600 } },
      ...
      if (
        callingAtCodes.some(
          ({ crsCode }) => !this.validateStationExists(crsCode, "high")
        )
      )
        return;
      if (!this.validateStationExists(terminatesAtCode, "low")) return;
  ...
  private async playInitialDepartureAnnouncement(
    options: IInitialDepartureAnnouncementOptions,
    download: boolean = false
  ): Promise<void> {
    ...
    if (callingAtCodes.length === 0) {
      ...
      files.push(
        { id: "the next station is", opts: { delayStart: 1000 } },
        `stations.high.${terminatesAtCode}`,
        `our final destination`
      );
    } else {
      ...
      props: {
        ...
        options: {
          terminatesAtCode: {
            ...
            options: AllStationsTitleValueMap.filter((s) =>
              this.AvailableStationNames.low.includes(s.value)
            ),
            type: "select",
          },
          callingAtCodes: {
        ...
        options: {
          ...
          changeForOverridden: {
            ...
            component: ({ activeState }) => {
              if (
                Object.keys(this.StationsWithForcedChangeHere).includes(
                  activeState.stationCode as string
                )
              ) {
                return (
                  <p className="warningMessage">
                    The "Change for" setting will have no effect for this
                    station.
                  </p>
                );
              }
          ...
          poiMessage: {
            ...
            component: ({ activeState }) => {
              if (
                Object.keys(this.StationsWithAttractions).includes(
                  activeState.stationCode as string
                )
              ) {
                return (
                  <p className="infoMessage">
                    This announcement will also contain additional information
                    which cannot be modified, relating to local points of
                    interest.
                  </p>
                );
              }
        ...
        buttonSections: {
          Safety: [
            ...
            },
            {
              label: "BTP (old)",
              play: this.playAudioFiles.bind(this, [
                "matt.if you see something that doesnt look right speak to staff or text",
              ]),
              download: this.playAudioFiles.bind(
                this,
                [
          ...
          General: [
            ...
            {
              label:
                "Oyster and contactless may only be used on certain routes",
              play: this.playAudioFiles.bind(this, [
          ...
          Disruption: [
            ...
            },
            {
              ...
              play: this.playAudioFiles.bind(this, [
                "disruption.this train is being held at a signal owing to congestion ahead",
              ]),
              download: this.playAudioFiles.bind(
                this,
                [
            ...
            },
            {
              ...
              play: this.playAudioFiles.bind(this, [
                "disruption.we apologise this train is being diverted from its normal route",
              ]),
              download: this.playAudioFiles.bind(
                this,
                [
          ...
          Legacy: [
            {
              ...
              play: this.playAudioFiles.bind(this, [
                "matt.you must wear a face covering on your jouney unless you are exempt",
              ]),
              download: this.playAudioFiles.bind(
                this,
                [
            ...
            },
            {
              ...
              play: this.playAudioFiles.bind(this, [
                "special.brighton mainline improvement 2018 geoff 1",
              ]),
              download: this.playAudioFiles.bind(
                this,
                ["matt.brighton mainline improvement 2018 geoff 1"],
                true
              ),
            },
            {
              ...
              play: this.playAudioFiles.bind(this, [
                "special.brighton mainline improvement 2018 geoff revised generic",
              ]),
              download: this.playAudioFiles.bind(
                this,
                [
                  "matt.brighton mainline improvement 2018 geoff revised generic",
                ],
                true
              ),
            },
            {
              label:
                "Brighton Mainline Improvements (2018) - Geoff (Revised and detailed)",
              play: this.playAudioFiles.bind(this, [
                "special.brighton mainline improvement 2018 geoff revised detailed",
              ]),
              download: this.playAudioFiles.bind(
                this,
                [
                  "matt.brighton mainline improvement 2018 geoff revised detailed",
                ],
                true
              ),
            },
