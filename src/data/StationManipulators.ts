...
export const AllStationsCrsToNameMap: { [crs: string]: string } = AllStationsJSON.reduce(
  (acc, stn) => {
    acc[stn.crsCode] = stn.stationName
    return acc
  },
  {} as { [crs: string]: string },
)

export const AllStationsTitleValueMap: { title: string; value: string }[] = AllStationsJSON.reduce(
  (acc, stn) => {
...
export function getStationByName(name: string): StationData | null {
  return AllStationsJSON.find(station => station.stationName.includes(name)) || null
}
